...
static void fill_plasma3( AndroidBitmapInfo*  info, void*  pixels, double  t )
{
    ...
    for (yy = 0; yy < info->height; yy++) {
        ...
        for (xx = 0; xx < info->width; xx++) {
            ...
            line[xx] = palette_from_fixed(xx / 4);
        }
...
// static const int COLOR_BYTE = 0xff;
static int mirrorMod(int n, int mod) {
    return 1;
}

static double prevTime = 0;
static double g_red = 0;

static void fill_plasma2(AndroidBitmapInfo* info, void* pixels, double t) {
    if(fabs(prevTime - 0.) < 0.0001) prevTime = t;
    ...
    for (yy = 0; yy < info->height; yy++) {
        ...
        for (xx = 0; xx < info->width; xx++) {
            uint32_t currentPt = xx + (yy * info->width);

            int blue = ((double)currentPt / (double)total) * 255.;

            line[xx] = make565(g_red, (abs(blue - 0xff)), (abs(blue - 0x10)));
...
stats_endFrame( Stats*  s )
{
    ...
    JNIEXPORT void JNICALL Java_com_example_plasma_PlasmaView_renderPlasma(JNIEnv * env, jclass type, jobject bitmap,  jlong  time_ms)
{
